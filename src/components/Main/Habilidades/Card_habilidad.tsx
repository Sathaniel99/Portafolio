...
export function Card_habilidad({ text, img, altImg, url, ariaLabel, bgClass }: Card_habilidadProp) {
    return (
        <Tooltip>
            <TooltipTrigger>
                <a className={`flex flex-col h-42 w-32 transition-all no-underline hover:-translate-y-1 border border-gray-700 hover:shadow-sm shadow-neutral-400 dark:shadow-neutral-950 ${bgClass} rounded-2xl m-1 p-2`} target="_blank" rel="noopener noreferrer" role="button" href={url} aria-label={ariaLabel}>
